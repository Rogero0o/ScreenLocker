 ...
 * <ul>
 * <li><b>Must not</b> try to change a jumping beans text in a textview before
 * calling
 ...
 * <li><b>Must not</b> try to use a jumping beans text in another view; it will
 * not
 * animate. Just create another jumping beans animation for each new
 * view</li>
 * <li><b>Must not</b> use more than one JumpingBeans instance on a single
 * TextView, as
 * the first cleanup operation called on any of these JumpingBeans will also
 * cleanup
 * all other JumpingBeans' stuff. This is most likely not what you want to
 * happen in
 * some cases.</li>
 * <li><b>Should not</b> use JumpingBeans on large chunks of text. Ideally this
 * should
 * be done on small views with just a few words. We've strived to make it as
 * inexpensive
 * as possible to use JumpingBeans but invalidating and possibly relayouting a
 * large
...
public final class JumpingBeans {
    ...
    /**
     * The default fraction of the whole animation time spent actually
     * animating.
    ...
    public void stopJumping() {
        ...
        if (tv != null) {
            ...
            if (text instanceof Spanned) {
                CharSequence cleanText = removeJumpingBeansSpans(
                        (Spanned) text);
     ...
     * the desired jumping beans effect. With this builder you can easily
     * append
    ...
    public static class Builder {
        ...
        public Builder appendJumpingDots(TextView textView) {
            ...
            CharSequence text = !TextUtils.isEmpty(textView.getText())
                                ? textView.getText()
                                : "";
            if (text.length() > 0 &&
                    text.subSequence(text.length() - 1, text.length())
                        .equals("…")) {
                text = text.subSequence(0, text.length() - 1);
            }

            if (text.length() < 3 || !TextUtils.equals(
                    text.subSequence(text.length() - 3, text.length()),
                    "...")) {
                text = new SpannableStringBuilder(text).append(
                        "...");  // Preserve spans in original text
            }
        ...
        public Builder makeTextJump(TextView textView, int startPos, int endPos) {
            if (textView == null || textView.getText() == null) {
                throw new NullPointerException(
                        "The textView and its text must not be null");
            }
            ...
            if (startPos < 0) {
                throw new IndexOutOfBoundsException(
                        "The start position must be non-negative");
            }

            this.text = textView.getText();
            if (endPos > text.length()) {
                throw new IndexOutOfBoundsException(
                        "The end position must be smaller than the text length");
            }
        ...
        /**
         * Sets the fraction of the animation loop time spent actually
         * animating.
        ...
        public Builder setAnimatedDutyCycle(float animatedRange) {
            if (animatedRange <= 0f || animatedRange > 1f) {
                throw new IllegalArgumentException(
                        "The animated range must be in the (0, 1] range");
            }
         ...
         *
         * @param loopDuration The jumping animation loop duration, in
         * milliseconds
         */
        public Builder setLoopDuration(int loopDuration) {
            if (loopDuration < 1) {
                throw new IllegalArgumentException(
                        "The loop duration must be bigger than zero");
            }
        ...
        /**
         * Sets the delay for starting the animation of every single dot over
         * the
         * start of the previous one, in milliseconds. The default value is
         * the loop length divided by three times the number of character
         * animated
         * by this instance of JumpingBeans.
         * <p/>
         * Only has a meaning when the animation is a wave.
         *
         * @param waveCharOffset The start delay for the animation of every
         * single
        ...
        public Builder setWavePerCharDelay(int waveCharOffset) {
            if (waveCharOffset < 0) {
                throw new IllegalArgumentException(
                        "The wave char offset must be non-negative");
            }
        ...
        public JumpingBeans build() {
            ...
            if (!wave) {
                jumpingBeans = new JumpingBeansSpan[] {
                        new JumpingBeansSpan(textView, loopDuration, 0, 0,
                                animRange) };
                sbb.setSpan(jumpingBeans[0], startPos, endPos,
                ...
                for (int pos = startPos; pos < endPos; pos++) {
                    JumpingBeansSpan jumpingBean = new JumpingBeansSpan(
                            textView, loopDuration, pos - startPos,
                            waveCharDelay, animRange);
                    sbb.setSpan(jumpingBean, pos, pos + 1,
                            Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
