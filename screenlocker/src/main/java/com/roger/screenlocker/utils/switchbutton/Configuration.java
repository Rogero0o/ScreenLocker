...
public class Configuration implements Cloneable {
    ...
    private int mOnColor = Default.DEFAULT_ON_COLOR, mOffColor
            = Default.DEFAULT_OFF_COLOR, mThumbColor
            = Default.DEFAULT_THUMB_COLOR, mThumbPressedColor
            = Default.DEFAULT_THUMB_PRESSED_COLOR;

    /**
     * space between View's border and thumb
     */
    private int mThumbMarginTop = 0, mThumbMarginBottom = 0, mThumbMarginLeft
            = 0, mThumbMarginRight = 0;
    ...
    /**
     * factor limit the minimum width equals almost (the height of thumb *
     * measureFactor)
     */
    ...
    public static Configuration getDefault(float density) {
        ...
        defaultConfiguration.setThumbMarginInPixel(
                defaultConfiguration.getDefaultThumbMarginInPixel());

        defaultConfiguration.mInsetBounds = new Rect(
                Default.DEFAULT_INNER_BOUNDS, Default.DEFAULT_INNER_BOUNDS,
                Default.DEFAULT_INNER_BOUNDS, Default.DEFAULT_INNER_BOUNDS);
    ...
    public void setThumbDrawable(Drawable thumbDrawable) {
        if (thumbDrawable == null) {
            throw new IllegalArgumentException(
                    "thumb drawable can not be null");
        }
    ...
    public void setThumbMarginInPixel(int marginInPixel) {
        setThumbMarginInPixel(marginInPixel, marginInPixel, marginInPixel,
                marginInPixel);
    }
    ...
    public void setThumbWidthAndHeight(int width, int height) {
        setThumbWidthAndHeightInPixel((int) (width * density),
                (int) (height * density));
    }
    ...
    public Drawable getThumbDrawableWithFix() {
        if (mThumbDrawable != null) {
            ...
            int[] stateSet = null;
            try {
                Field stateField = View.class.getDeclaredField(
    ...
    public boolean needShrink() {
        return this.mInsetBounds.left + this.mInsetBounds.right +
                this.mInsetBounds.top + this.mInsetBounds.bottom != 0;
    }
