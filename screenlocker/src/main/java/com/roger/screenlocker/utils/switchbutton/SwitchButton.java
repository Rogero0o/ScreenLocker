package com.roger.screenlocker.utils.switchbutton;
...
public class SwitchButton extends CompoundButton {
    ...
    public SwitchButton(Context context, AttributeSet attrs, int defStyle) {
        ...
        mConf.setThumbMarginInPixel(
                ta.getDimensionPixelSize(R.styleable.SwitchButton_thumb_margin,
                        mConf.getDefaultThumbMarginInPixel()));
        mConf.setThumbMarginInPixel(ta.getDimensionPixelSize(
                R.styleable.SwitchButton_thumb_marginTop,
                mConf.getThumbMarginTop()), ta.getDimensionPixelSize(
                R.styleable.SwitchButton_thumb_marginBottom,
                mConf.getThumbMarginBottom()), ta.getDimensionPixelSize(
                R.styleable.SwitchButton_thumb_marginLeft,
                mConf.getThumbMarginLeft()), ta.getDimensionPixelSize(
                R.styleable.SwitchButton_thumb_marginRight,
                mConf.getThumbMarginRight()));
        mConf.setRadius(ta.getInt(R.styleable.SwitchButton_radius,
                Configuration.Default.DEFAULT_RADIUS));

        mConf.setThumbWidthAndHeightInPixel(
                ta.getDimensionPixelSize(R.styleable.SwitchButton_thumb_width,
                        -1),
                ta.getDimensionPixelSize(R.styleable.SwitchButton_thumb_height,
                        -1));

        mConf.setMeasureFactor(
                ta.getFloat(R.styleable.SwitchButton_measureFactor, -1));

        mConf.setInsetBounds(
                ta.getDimensionPixelSize(R.styleable.SwitchButton_insetLeft, 0),
                ta.getDimensionPixelSize(R.styleable.SwitchButton_insetTop, 0),
                ta.getDimensionPixelSize(R.styleable.SwitchButton_insetRight,
                        0),
                ta.getDimensionPixelSize(R.styleable.SwitchButton_insetBottom,
                        0));

        int velocity = ta.getInteger(R.styleable.SwitchButton_animationVelocity,
                -1);
    ...
    private void initView() {
        mConf = Configuration.getDefault(
                getContext().getResources().getDisplayMetrics().density);
        mTouchSlop = ViewConfiguration.get(getContext()).getScaledTouchSlop();
        mClickTimeout = ViewConfiguration.getPressedStateDuration() +
                ViewConfiguration.getTapTimeout();
        mAnimationController = AnimationController.getDefault()
                                                  .init(mOnAnimateListener);
        mBounds = new Rect();
        if (SHOW_RECT) {
    ...
    private void fetchDrawableFromAttr(TypedArray ta) {
        ...
        mConf.setOffDrawable(
                fetchDrawable(ta, R.styleable.SwitchButton_offDrawable,
                        R.styleable.SwitchButton_offColor,
                        Configuration.Default.DEFAULT_OFF_COLOR));
        mConf.setOnDrawable(
                fetchDrawable(ta, R.styleable.SwitchButton_onDrawable,
    ...
    private Drawable fetchDrawable(TypedArray ta, int attrId, int alterColorId, int defaultColor) {
        ...
        if (tempDrawable == null) {
            ...
            ((GradientDrawable) tempDrawable).setCornerRadius(
                    this.mConf.getRadius());
    ...
    private Drawable fetchThumbDrawable(TypedArray ta) {
        ...
        Drawable tempDrawable = ta.getDrawable(
                R.styleable.SwitchButton_thumbDrawable);
        if (tempDrawable != null) {
            return tempDrawable;
        }

        int normalColor = ta.getColor(R.styleable.SwitchButton_thumbColor,
                Configuration.Default.DEFAULT_THUMB_COLOR);
        int pressedColor = ta.getColor(
                R.styleable.SwitchButton_thumbPressedColor,
                Configuration.Default.DEFAULT_THUMB_PRESSED_COLOR);
    ...
    public void setConfiguration(Configuration conf) {
        ...
        mConf.setThumbMarginInPixel(conf.getThumbMarginTop(),
                ...
                conf.getThumbMarginRight());
        mConf.setThumbWidthAndHeightInPixel(conf.getThumbWidth(),
                conf.getThumbHeight());
    ...
    /**
     * return a REFERENCE of configuration, it is suggested that not to change
     * that
     */
    ...
    private void setup() {
        ...
        if (this.getMeasuredWidth() > 0 && this.getMeasuredHeight() > 0) {
            mSaveLayerZone = new RectF(0, 0, this.getMeasuredWidth(),
    ...
    private void setupSafeZone() {
        ...
        if (w > 0 && h > 0) {
            ...
            left = getPaddingLeft() + (mConf.getThumbMarginLeft() > 0
                                       ? mConf.getThumbMarginLeft()
                                       : 0);
            right = w - getPaddingRight() - (mConf.getThumbMarginRight() > 0
                                             ? mConf.getThumbMarginRight()
                                             : 0) + (-mConf.getShrinkX());
            top = getPaddingTop() + (mConf.getThumbMarginTop() > 0
                                     ? mConf.getThumbMarginTop()
                                     : 0);
            bottom = h - getPaddingBottom() - (mConf.getThumbMarginBottom() > 0
                                               ? mConf.getThumbMarginBottom()
                                               : 0) + (-mConf.getShrinkY());
            mSafeZone.set(left, top, right, bottom);

            mCenterPos = mSafeZone.left +
                    (mSafeZone.right - mSafeZone.left - mConf.getThumbWidth()) /
                            2;
        }
        else {
            mSafeZone = null;
    ...
    private void setupThumbZone() {
        ...
        if (w > 0 && h > 0) {
            ...
            int left, right, top, bottom;
            left = mIsChecked
                   ? (mSafeZone.right - mConf.getThumbWidth())
                   : mSafeZone.left;
    ...
    private int measureWidth(int measureSpec) {
        ...
        int minWidth = (int) (mConf.getThumbWidth() * mConf.getMeasureFactor() +
                getPaddingLeft() + getPaddingRight());
        int innerMarginWidth = mConf.getThumbMarginLeft() +
                mConf.getThumbMarginRight();
        ...
        measuredWidth += (mConf.getInsetBounds().left +
                mConf.getInsetBounds().right);
    ...
    private int measureHeight(int measureSpec) {
        ...
        int minHeight = mConf.getThumbHeight() + getPaddingTop() +
                getPaddingBottom();
        int innerMarginHeight = mConf.getThumbMarginTop() +
                mConf.getThumbMarginBottom();
        ...
        measuredHeight += (mConf.getInsetBounds().top +
                mConf.getInsetBounds().bottom);
    ...
    @Override protected void onDraw(Canvas canvas) {
        ...
        if (mBounds != null && mConf.needShrink()) {
            ...
            canvas.translate(mConf.getInsetBounds().left,
                    mConf.getInsetBounds().top);
        }
    ...
    private boolean notStatableDrawable() {
        boolean thumbStatable
                = (mConf.getThumbDrawable() instanceof StateListDrawable);
        boolean onStatable
                = (mConf.getOnDrawable() instanceof StateListDrawable);
        boolean offStatable
                = (mConf.getOffDrawable() instanceof StateListDrawable);
    ...
    private int calcAlpha() {
        ...
        if (mSafeZone == null || mSafeZone.right == mSafeZone.left) {
            ...
            int backWidth = mSafeZone.right - mConf.getThumbWidth() -
                    mSafeZone.left;
            if (backWidth > 0) {
    ...
    class SBAnimationListener implements OnAnimateListener {
        ...
        @Override public boolean continueAnimating() {
            return mThumbZone.right < mSafeZone.right &&
                    mThumbZone.left > mSafeZone.left;
        }


        @Override public void onFrameUpdate(int frame) {
            moveThumb(frame);
            postInvalidate();
        }


        @Override public void onAnimateComplete() {
